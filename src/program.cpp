...
void execute_kernel(const program_t d_progs, const float *data, float *y_pred,
  ...
  for (uint64_t pid = 0; pid < n_progs; ++pid) {
    ...
    node* const prog_nodes = curr_p->nodes;
...
void execute(const program_t &d_progs, const int n_rows, const int n_progs,
  ...
  execute_kernel(d_progs, data, y_pred, n_rows, n_progs);
}
...
void find_batched_fitness(int n_progs, program_t d_progs, float *score,
  ...
  std::vector<float> y_pred_batch((uint64_t)n_rows * (uint64_t)batch_size);
  ...
  for (int batch_start = 0; batch_start < n_progs; batch_start += batch_size) {
    ...
    // Execute current batch of programs
    execute(d_progs + batch_start, n_rows, current_batch_size, data, y_pred_batch.data());
    
    // Compute error for current batch
    compute_metric(n_rows, current_batch_size, y, y_pred_batch.data(), 
                  sample_weights, score + batch_start, params);
  }
}
...
void set_batched_fitness(int n_progs, std::vector<program> &h_progs,
  ...
  for (int i = 0; i < n_progs; ++i) {
    fitness_ptrs[i] = &(h_progs[i].raw_fitness_);
  }
  
  // Call find_batched_fitness with pointers to the actual fitness fields
  find_batched_fitness(n_progs, h_progs.data(), fitness_ptrs.data(), params, n_rows,
