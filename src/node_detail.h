...
inline float evaluate_node(const node &n, const float *data, const uint64_t stride,
  ...
  if (__builtin_expect(n.t == node::type::constant, true)) {
    return n.u.val;
  } else if (__builtin_expect(n.t == node::type::variable, true)) {
    return data[(stride * n.u.fid) + idx];
  } else {
    // note: reordered cases to put most common operations first
    switch (n.t) {
    // binary operators
    case node::type::add:
      return in[0] + in[1];
